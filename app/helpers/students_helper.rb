...
module StudentsHelper
  def course_card_classes(selected_priority, course_disabled)
    ...
    classes << priority_color_border(selected_priority) if selected_priority
  ...
  def priority_button_content(priority, selected)
    return tag.span(t("students.show.prio.#{priority}")) unless selected
  ...
  def check_icon
    tag.svg(class: '-ml-0.5 size-5', viewBox: '0 0 20 20', fill: 'currentColor', 'aria-hidden': 'true',
            'data-slot': 'icon') do
  ...
  def priority_color_text(priority)
    map = {
  ...
  def priority_color_border(priority)
    map = {
      top: 'border-emerald-600 ring-2 ring-emerald-600',
      mid: 'border-amber-600 ring-2 ring-amber-600',
      low: 'border-rose-600 ring-2 ring-rose-600'
    }
